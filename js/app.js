...
var menuItems = ["chicken nuggets", "cheeseburger", "iced coffee"];
...
myButton.addEventListener("click", replaceQuote);

function replaceQuote(){
	var theText = document.createElement("p");
	theText.innerHTML = myQuote;
	displayQuote.appendChild(theText);
}
...
var newQuote = document.getElementById("random");
newQuote.addEventListener("click", pickQuote); 

function pickQuote(){
	var randomQuote = document.createElement("p");
 	randomQuote.innerHTML = quotes[Math.floor(Math.random() * quotes.length)];
 	displayQuotes.appendChild(randomQuote);
...
var showMessage = document.getElementById("showHide");
showMessage.addEventListener("click", seeMessage);   

function seeMessage() {
    var checkMessage = document.getElementById("showmoney");
    if (checkMessage.style.display === 'none') {
